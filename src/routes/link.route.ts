...
link_router.post('/shorten', body('originalURL').not().isEmpty().withMessage('The originalURL Parameter is required.'),
    check_for_bad_request,
    getUserID, shorten_url);

link_router.get('/search', verifyUser, body('query').not().isEmpty().withMessage('Write One or More Words.'),
    check_for_bad_request,
    search_for_URL);

link_router.post('/update', verifyUser, body('linkID').not().isEmpty().withMessage('The linkID Parameter is required.'),
    check_for_bad_request,
    update_link_info
)

link_router.post('/delete', body('linkID').not().isEmpty().withMessage('The linkID Parameter is required.'),
    check_for_bad_request,
